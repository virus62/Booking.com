...
// potrebne funkcije za kod, glavni dio na liniji 80
...
export const find_and_check_user = (user) => { // funckija koja traži je li korisnik unjeo točnu e-poštu i lozinku, vraća false ako ne nađe, a user_info ako nađe
    ...
    submit_button.addEventListener("click", (e) => {
        ...
        if (enabled.length > 4) { // enabled označuje formu koju trenutačno gledamo, ako je duljina forme 4, to znači da se nalazimo na registraciji
            ...
            if (login_user === false) {
                ...
                const user_list = JSON.parse(localStorage.getItem("registered_users"));
                for (const user of user_list) {
                    if (user.email == user_info.email && user.password == user_info.password) {
